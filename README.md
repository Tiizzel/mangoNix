...
Follow these steps to deploy this configuration onto a freshly installed NixOS machine.

### 1. Install NixOS
1. Boot from the official NixOS installer (graphical or minimal ISO).
2. Complete the standard installer steps.
3. Boot into your new NixOS installation.
...
mangoNix uses declarative NixOS module options. You don't have to search and replace usernames in multiple files.

Open `modules/users/user.nix` (or your host configuration):
...
> **Note on Graphics Drivers:** If your computer uses an **NVIDIA** or **Intel** GPU instead of AMD, adjust `modules/hardware/drivers.nix` to load the appropriate video drivers before rebuilding.
...
Nix will download all flake inputs, compile packages, generate your Home Manager configuration, and automatically symlink all dotfiles.
